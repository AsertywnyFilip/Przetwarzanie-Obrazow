...
void histogram(struct obraz *obrazek)
{
	FILE *plik;
	int *wyniki;

	wyniki = calloc(obrazek->glebia + 1, sizeof(int));
	for (int i = 0; i < obrazek->wys; i++)
	{
		for (int j = 0; j < obrazek->szer; j++)
		{
			wyniki[obrazek->tablica[j][i]]++;
		}
	}


	plik = fopen("wykres.csv", "w");
	for (int i = 0; i < obrazek->glebia + 1; i++)
	{
		fprintf(plik, "%d;%d \n", i, wyniki[i]);
	}
	fclose(plik);
